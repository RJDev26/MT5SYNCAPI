...
namespace OTS.MobileAccountingAPI.Controllers
{
    ...
    public class MasterController : ControllerBase
    {
        ...
        [HttpGet("exchanges")]
        public async Task<IActionResult> GetExchanges(CancellationToken ct = default)
        {
            var rows = await _masterService.GetMasterListAsync("Exchange", ct);
            return Ok(rows);
        }
        ...
        {
            if (request.CreatedBy == null || request.CreatedBy==0) { request.CreatedBy = 1; }  
